...
class Mail {
    ...
    static function sendActivation(Cliente $usuario) {
        $asunto = 'Active su cuenta de Pelulife';
        $jwt = \Firebase\JWT\JWT::encode($usuario->getCorreo(), App::JWT_KEY);
        // $enlace = Util::url() . 'doactivar.php?id='. $usuario->getId() .'&code=' . $jwt;
        $enlace = App::BASE.'user/doActivar?id='.$usuario->getId();
        $mensaje = "Activación para:  ". $usuario->getNombre();
